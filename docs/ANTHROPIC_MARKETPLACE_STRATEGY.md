# Anthropic Marketplace Strategy: The Reliability & Context Engineering Layer

> Historical research note: this strategy file may reference outdated pricing and distribution assumptions. Use `docs/COMMERCIAL_TRUTH.md` for current pricing and traction claims.

## Marketplace Positioning
MCP Memory Gateway is the **Enterprise Control Plane** for Claude-based agentic workflows. We solve the "Vibe Coding" security crisis by providing a verifiable, context-engineered runtime.

## Value Proposition for Marketplace Customers
1. **Context Engineering Moat:** Move beyond raw prompts. Use our Semantic Context Packs to give Claude high-density proprietary knowledge.
2. **Zero-Trust Agentic Guardrails:** Mitigate supply-chain risks (Pentagon-grade concerns) by enforcing machine-readable prevention rules auto-generated from human feedback.
3. **Budget-Safe Autonomy:** Fully compatible with Anthropic's annual spend commitment model. Our `budget-guard` ensures agents never exceed committed marketplace spend.

## Product Categorization (Listing Plan)
- **Primary Category:** Agent Skills & Infrastructure
- **Secondary Category:** Developer Tools / AI Reliability
- **Tags:** Context Engineering, RLHF, DPO, Agentic Governance, Claude 3.5 Sonnet, MCP Server

## Technical Integration (The Listing Payload)
- **Delivery Format:** MCP Server (Standard) + OpenAPI Spec (for Claude Cowork).
- **Security Evidence:** All listed skills are verified by our `VERIFICATION_EVIDENCE.md` audit trail.
- **Pricing:** The current public self-serve offer is Pro at `$29/mo` recurring per `docs/COMMERCIAL_TRUTH.md`. Hosted gateway access is pilot/by-request.

## Strategic Improvement: "Vibe-to-Verification" (V2V)
The product will now feature a **V2V Pipeline**:
1. **Vibe:** User gives a "thumbs down" because the agent "felt wrong" (Vibe coding signal).
2. **Context Engineering:** We capture the exact semantic state during that failure.
3. **Verification:** We auto-generate a `CLAUDE.md` rule that converts that vibe into a hard architectural constraint.
